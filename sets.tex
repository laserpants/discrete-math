...
\noindent For any set $ A $ : $ A \times \varnothing = \varnothing = \varnothing \times A $. Just like the set-theoretic difference, the cartesian product is not commutative. Specifically, $ A \times B $ is not equal to $ B \times A $, unless 
\begin{packed_items}
\item $ A = B $, or 
...
As a result of this, axiomatic set theories were developed, such as the Zermelo-Fraenkel set theory. Today, ZFC (where the C refers to the \begin{em}Axiom of Choice\end{em}) serves as a standard foundation of mathematics.
In practical terms, we have to be careful when talking about sets of other sets. In particular, there is no ``set of all sets." In axiomatic set theory, this would instead be referred to as a \begin{em}proper class\end{em}. \\
